...
void test_tRow_all<RE__,RE__,CPX__>::test_assign() {
	...
	// from real scalar
	{
		...
		*j = f;
		CPPUNIT_ASSERT(std::all_of(j->begin(),j->end(),[&f](const RE__ i){return i==f;}));
	}
	...
	// from complex scalar
	{
		...
		*j = f;
		CPPUNIT_ASSERT(std::all_of(j->begin(),j->end(),[&f](const RE__ i){return i==std::real(f);}));
	}
	...
	// from real row
	{
		...
		*j1 = *j2;
		auto i2=j2->cbegin();
		CPPUNIT_ASSERT(std::all_of(j1->begin(),j1->end(),[&i2](const RE__ i){return i==*i2++;}));
	}
	...
	// from complex row
	{
		...
		*j1 = *j2;
		auto i2=j2->cbegin();
		CPPUNIT_ASSERT(std::all_of(j1->begin(),j1->end(),[&i2](const RE__ i){return i==std::real(*i2++);}));
	}
